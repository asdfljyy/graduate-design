...
MIN_SBOXES=(1,8,13,24,30,36,43,50,56,66)#各轮活跃S盒数量下界
MAX_PR=[2 for i in range(10)]#P为各轮与活跃S盒下界吻合的最优差分概率,表中值为-log P
...
def write_cvc(Round,pr):#写文件,pr为猜测的最大差分概率
    f=open(r"OptiPr_%d.cvc"%(Round),"w",encoding="utf-8")
...
def process(Round):
    start=time.time()
    bound=MAX(MAX_PR[0:Round-1])#此下界适用于连续运行函数的情况，运行单个函数时下界为2*活跃S盒数量
    if bound<2*MIN_SBOXES[Round-1]:
        bound=2*MIN_SBOXES[Round-1]
    for i in range(bound,128*Round):
        write_cvc(Round,i)
        #print(i,'stp start')
        os.system("stp OptiPr_%d.cvc > result_%d.txt"%(Round,Round))
        #print(i,'stp finish')
    ...
    for i in range(length):
        ...
        if lines[i][8:10]=='P_':
            neg_logP[int(lines[i][10])]=int(lines[i][14:18],16)
        if lines[i][8:10]=='PR':
            P=int(lines[i][13:17],16)
    print('%s轮活跃S盒数量下界为%d,与之吻合的最优差分特征概率：2^-%d'%(Round,MIN_SBOXES[Round-1],P))
    print('最优差分特征：')
    for i in range(Round):
        print('第%d轮 '%(i+1),delta[i][0],'-->',delta[i][1])
    print('每轮差分特征概率的对数负值：',neg_logP)#-log p
    end=time.time()
